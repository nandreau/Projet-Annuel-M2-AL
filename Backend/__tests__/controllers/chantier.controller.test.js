...
describe('Chantier Controller', () => {
  describe('create', () => {
    it('should create a new chantier and respond with success', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Chantier créé avec succès', data: chantier });
    });
  ...
  describe('findAll', () => {
    it('should return all chantiers for admin user', async () => {
      ...
      const chantierList = [{ id: 101, dataValues: {} }, { id: 102, dataValues: {} }];
      Chantier.findAll.mockResolvedValue(chantierList);
      const extractIntervenants = require('../../app/utils/chantier').extractIntervenants;
    ...
    it('should handle errors and respond with 500', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Erreur lors de la récupération : Query failed' });
    });
  });
  ...
  describe('findAssignedUsers', () => {
    it('should return all users assigned to the chantier', async () => {
      req.params = { id: '5' };
      const users = [{ id: 10, name: 'Alice' }, { id: 11, name: 'Bob' }];
  ...
  describe('findOne', () => {
    ...
    it('should handle errors and respond with 500', async () => {
      ...
      await chantierController.findOne(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ message: 'Erreur lors de la récupération : DB error' });
    });
  });
  ...
  describe('update', () => {
    it('should update chantier data and return updated chantier', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Mise à jour réussie', data: updatedChantier });
    });
    ...
    it('should handle errors and return 400', async () => {
      ...
      await chantierController.update(req, res);
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.json).toHaveBeenCalledWith({ message: 'Erreur lors de la mise à jour : Update error' });
    });
  });
