...
const { Assignment } = require("../../app/models");
...
describe("Assignment Controller", () => {
  ...
  describe("create", () => {
    it("should create a new assignment and link users, then respond with success message", async () => {
      ...
      expect(Assignment.findByPk).toHaveBeenCalledWith(
        assignmentInstance.id,
        expect.any(Object),
      );
  ...
  describe("findOne", () => {
    ...
    it("should return 404 if assignment not found", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({
        message: "Aucun assignement trouvé avec cet identifiant.",
      });
    });
  });
  ...
  describe("update", () => {
    ...
    it("should return 404 if assignment to update is not found", async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(404);
      expect(res.json).toHaveBeenCalledWith({
        message: "Aucun assignement trouvé à mettre à jour.",
      });
    });
  ...
  describe("delete", () => {
    ...
    it("should return 404 if assignment not found", async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(404);
      expect(res.json).toHaveBeenCalledWith({
        message: "Aucun assignement trouvé à supprimer.",
      });
    });
  });
});
